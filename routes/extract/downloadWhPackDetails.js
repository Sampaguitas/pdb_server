    ...
    if (!screenId || !projectId) {
      ...
      .exec(async function(errProject, resProject) {
        if (errProject) {
          ...
          //hide Ids
          worksheet.getColumn('A').hidden = true; //whcollipackId
...
function getLines (resProject, fieldnames, screenId) {
    let arrayBody = [];
    let arrayRow = [];
        if (resProject.whcollipacks) {
          resProject.whcollipacks.map(whcollipack => {
            arrayRow = [];
            arrayRow.push({
              val: whcollipack._id, //collipackId
              col: 1,
              type: 'String',
              align: 'left',
              edit: true
            ...
            fieldnames.map( (fieldname, index) => {
              switch(fieldname.fields.fromTbl) {
                case 'collipack':
                  arrayRow.push({
                    val: getValue(fieldname.fields.name, whcollipack),
                    col: index + 2,
