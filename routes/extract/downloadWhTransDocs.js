        ...
        if (errProject) {
          ...
          if (!_.isEmpty(myHeaders)) {
            ...
            myHeaders.map(function (header) {
              ...
              myFgColour = header.col < 7 ? { argb: 'A8052C'} : { argb: '0070C0'}
              with (cell) {
          ...
          worksheet.autoFilter = `A2:${alphabet(resProject.fieldnames.length + 6)}2`;
          ...
          worksheet.getColumn('E').hidden = true; //whpackitems
          worksheet.getColumn('F').hidden = true; //mirId
...
function getLines (resProject, fieldnames, screenId, locale) {
    ...
    if (resProject.picktickets) {
      resProject.picktickets.map(pickticket => {
        if (pickticket.pickitems) {
          pickticket.pickitems.map(pickitem => {
            if (pickitem.whpackitems) {
              pickitem.whpackitems.map(whpackitem => {
                ...
                arrayRow.push({
                  val: whpackitem._id, //whpackitemId
                  col: 5,
                  type: 'String',
                  align: 'left',
                  edit: true
                ...
                fieldnames.map( (fieldname, index) => {
                  switch(fieldname.fields.fromTbl) {
                    ...
                    case 'packitem':
                      arrayRow.push({
                        val: getValue(fieldname.fields.name, whpackitem),
                        col: index + 7,
                        type: fieldname.fields.type,
                        align: fieldname.align,
                        edit: fieldname.edit
