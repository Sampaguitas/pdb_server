...
router.post('/', upload.single('file'), function (req, res) {
      ...
      if (errFieldNames || !resFieldNames) {
        ...
        workbook.xlsx.load(file.buffer).then(wb => { //edited
          ...
          if (rowCount < 3) { //2
            ...
            return res.status(400).json({
              message: 'try to upload less rows than 1000 rows at the time',
              rejections: rejections,
              nProcessed: nProcessed,
              nRejected: nRejected,
              nEdited: nEdited
            ...
            (async function() {
              for (let row = 3; row < rowCount + 1 ; row++) { //2
                ...
                //assign ColliPack Ids
                tempColliPack.projectId = projectId;
                tempColliPack._id = clean(worksheet.getCell('A' + row).value);
                ...
                await Promise.all(colPromises).then( async () => {
                  rowPromises.push(update(row, tempColliPack));
                }).catch(errPromises => {
  ...
  function update(row, tempColliPack) {
    return new Promise (function (resolve) {
      ColliPack.findByIdAndUpdate(tempColliPack._id, tempColliPack, { new: true }, function(errNewColliPack, resNewColliPack){
          if (errNewColliPack || !resNewColliPack) {
            resolve({
              ...
              isEdited: false,
              isAdded: false,
              reason: 'Fields from Table ColliPack could not be saved.'
            });
          } else {
            resolve({
              row: row,
